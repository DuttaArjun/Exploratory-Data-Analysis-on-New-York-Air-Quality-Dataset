...
# _Analyzing the Structure of the Data_

```{r ,echo=F ,message=F,warning=F,results='hide'}
#Packages Deployed
load.libraries <- c('e1071','dplyr','datasets','VIM')
#if not present identify and download
install.lib <- load.libraries[!load.libraries %in% installed.packages()]
sapply(install.lib, install.packages)
#Load the packages
...
# _Outlier Detection and Treatment_
...
```{r,echo=F,comment=NA}
par(mfrow=c(1,2))
boxplot(airquality$Ozone,col = "Red",main = "Boxplot of Ozone",outcol="Blue",outpch=19,boxwex=0.7,range = 1.5)
hist(airquality$Ozone,col = "Green",main = "Histogram of Ozone", xlab = "Observations",breaks = 15)
...
```{r,echo=F,comment=NA}
par(mfrow=c(1,2))
boxplot(airquality$Solar.R,col = "Red",main = "Boxplot of Solar.R",outcol="Blue",outpch=19,boxwex=0.7,range = 1.5)
hist(airquality$Solar.R,col = "Green",main = "Histogram of Solar.R", xlab = "Observations",breaks = 15)
...
```{r,echo=F,comment=NA}
par(mfrow=c(1,2))
boxplot(airquality$Wind,col = "Red",main = "Boxplot of Wind",outcol="Blue",outpch=19,boxwex=0.7,range = 1.5)
hist(airquality$Wind,col = "Green",main = "Histogram of Wind", xlab = "Observations",breaks = 15)
...
```{r,echo=F,comment=NA}
par(mfrow=c(1,2))
boxplot(airquality$Temp,col = "Red",main = "Boxplot of Temp",outcol="Blue",outpch=19,boxwex=0.7,range = 1.5)
hist(airquality$Temp,col = "Green",main = "Histogram of Temp", xlab = "Observations",breaks = 15)
...
# _Missing Value Detection and Treatment_
...
__Patterns__
```{r, echo=F,comment=NA,message=FALSE,warning=FALSE,error=FALSE}
library(mice)
md.pattern(Data)
```
...
```{r, echo=F,comment=NA,message=FALSE,warning=FALSE,error=FALSE}
library(VIM)
aggr_plot <-  aggr(Data[c(1,2)], col=c('navyblue','red'), numbers=TRUE, sortVars=TRUE, labels=names(Data[c(1,2)]), cex.axis=.7, gap=3, ylab=c("Histogram of missing data","Pattern"))
```
...
```{r,echo=FALSE,comment=NA,message=FALSE,warning=FALSE,error=FALSE,results="hide"}
...
# _Feature Engineering and Analysis_
...
```{r,echo=F,fig.width=8, fig.height=6,comment=NA}
Viz(Data$Ozone)
skewness(Data$Ozone)
```
